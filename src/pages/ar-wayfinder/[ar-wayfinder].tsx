...
import { ArWayfinderPage } from '@/src/features/ar-wayfinder/presentation/page';
...
export default function ArWayfinder() {
  ...
  return (
    <>
      <Head>
        <title>{`Ar Wayfinder ${title ? `| ${title}` : ''}`}</title>
      </Head>
      <ArWayfinderPage />
    </>
  );
...
ArWayfinder.getLayout = function getLayout(page: ReactElement) {
  return <BaseLayout isShowBackgroundImage={false}>{page}</BaseLayout>;
};
